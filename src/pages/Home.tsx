...
const Home = () => {
  ...
  const [questionSettings, setQuestionSettings] = useState<QuestionSettings>({
    ...
    question2: 'Что хочется запомнить из сегодняшнего дня?',
  });
  ...
  const randomQuestions = [
    ['Какие эмоции сопровождали сегодня?', 'Что запомнилось больше всего?'],
    ['Какие чувства были в течение дня?', 'Какой момент хочется сохранить?'],
    ['Какое настроение преобладало?', 'За что есть благодарность сегодня?'],
    ['Что происходило во внутреннем мире?', 'Какие открытия произошли сегодня?'],
    ['Какие чувства возникали?', 'Что принесло радость?'],
    ['Как проходил день эмоционально?', 'Что важного произошло?'],
  ];
    ...
    setTimeout(() => {
      if (Notification.permission === 'granted') {
        new Notification('📝 Время записи эмоций!', {
          body: 'Как прошёл день? Поделитесь своими эмоциями',
          icon: '/favicon.svg',
        });
      }
      scheduleNotification();
        ...
        <div className="min-h-screen flex items-center justify-center p-6 pb-24 bg-white">
          <Card className="max-w-md w-full p-8 text-center animate-fade-in border-gray-200">
            ...
            <p className="text-gray-600 text-sm leading-relaxed mb-6">
              Спасибо! Форма уже заполнена сегодня.
            </p>
            <Button
              onClick={handleEdit}
              variant="outline"
